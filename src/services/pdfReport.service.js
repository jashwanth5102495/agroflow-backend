...
exports.generateDailyReportPDF = exports.fetchDailyReportData = void 0;
 ...
 */
const fetchDailyReportData = async (shopId, dateStr) => {
    ...
    let totalCash = 0;
    let totalCredit = 0;
    const salesDetails = await Promise.all(sales.map(async (sale) => {
        totalSales += sale.total || 0;
        totalCash += sale.amountPaid || 0;
        totalCredit += sale.amountDue || 0;
        const items = await SaleItem_1.SaleItem.find({ saleId: sale._id }).lean();
        return {
    ...
    const creditAccounts = await CreditAccount_1.CreditAccount.find({
        shopId: new mongoose_1.default.Types.ObjectId(shopId),
        balance: { $gt: 0 }
    })
        .populate('farmerId', 'name phone village')
        ...
        .limit(20)
        .lean();
    const creditFarmers = creditAccounts.map((ca) => ({
    ...
    const lowStockItems = await Inventory_1.Inventory.aggregate([
...
exports.fetchDailyReportData = fetchDailyReportData;
/**
